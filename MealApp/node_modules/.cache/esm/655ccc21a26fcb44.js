let Axios;_a26‍.x([["createUser",()=>createUser],["loginUser",()=>loginUser],["getUserStatus",()=>getUserStatus]]);_a26‍.w("axios",[["default",["Axios"],function(v){Axios=v}]]);

//testing
// export async function fn1() {

//     const result = await Axios({
//         method: 'get',
//         url: 'http://localhost:3000/public/authors/',
//     })
//     // result = JSON.stringify(result, null, 4);
//     console.log(result);

// };

// export async function fn2() {
//     try{
//         const result = await Axios({
//             method: 'post',
//             url: 'http://localhost:3000/public/authors/',
//             data: {
//                 data: "Lord of the Rings"

//             }
//         })
//         // result = JSON.stringify(result);
//         console.log(result);
//     }catch(error){
//         console.log(error);
//     }

//     // axios.post('http://localhost:3000/public/', {
//     //         username: 'api',
//     //         password: 'MY_PASSWORD',
//     //         grant_type: 'MY_GRANT_TYPE'
//     //     }, {
//     //         headers: {
//     //             'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
//     //         }
//     //     })
//     //     .then(response => {
//     //         console.log(response)
//     //     })
//     //     .catch(error => {
//     //         console.log(error.response)
//     //     });





// }
// fn2()
// fn1()

       async function createUser(username, password, email) {
    try {
        const result = await Axios({
            method: 'post',
            url: 'http://localhost:3000/account/create',
            data: {
                "name": username,
                "pass": password,
                "data": {
                  "email":email
                }
              }
        })
        // result = JSON.stringify(result);
        // console.log(result);
    } catch (error) {
        _a26‍.g.console.log(error);
    }
}

       async function loginUser(username, password) {
    try {
        const result = await Axios({
            method: 'post',
            url: 'http://localhost:3000/account/login',
            data: {
                "name": username,
                "pass": password
              }
        })
        // result = JSON.stringify(result);
        _a26‍.g.console.log(result.data);
    } catch (error) {
        _a26‍.g.console.log(error);
    }
}


       async function getUserStatus(jwt) {
    try {
        const result = await Axios({
            method: 'get',
            url: 'http://localhost:3000/account/status',
            headers:{
                "Authorization": "Bearer "+jwt
            },
        })
        // result = JSON.stringify(result);
        _a26‍.g.console.log(result);
    } catch (error) {
        _a26‍.g.console.log(error);
    }
}

// createUser("namex","passx","xxx@x.com")
// loginUser("namex","passx")
getUserStatus('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoibmFtZXgiLCJkYXRhIjp7ImVtYWlsIjoieHh4QHguY29tIn0sImlhdCI6MTU3MTAwMzA0NiwiZXhwIjoxNTczNTk1MDQ2fQ._GVOTuIYfiOqWqQhu9J9CoC-k_moVKa4_HbuLx4wTVI');
